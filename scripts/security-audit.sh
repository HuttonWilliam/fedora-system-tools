#!/bin/bash
# ========================================================================
# Fedora System Tools - Security Audit
...
# Security Audit Tool for Fedora
...
${CYAN}Fedora Security Audit Tool${NC}
...
# ──────────────────────────────────────────────
# Check Firewall Status (Firewalld / UFW)
# ──────────────────────────────────────────────
audit_firewall() {
    print_section "Firewall Configuration"
    
    # Check Firewalld (Fedora Default)
    if command -v firewall-cmd &>/dev/null; then
        if systemctl is-active --quiet firewalld; then
            print_status "Firewalld is ACTIVE"
            echo -e "\n${BLUE}Active firewall zones/services:${NC}"
            firewall-cmd --get-active-zones | while read -r line; do
                echo "  $line"
                echo "  $line" >> "$AUDIT_LOG"
            done
        else
            print_error "Firewalld is INACTIVE - Consider enabling it"
        fi
        ...
        print_warning "No standard firewall manager detected"
    fi
...
audit_sudo() {
    ...
    # List users in wheel group (Fedora's equivalent to sudo group)
    echo -e "\n${BLUE}Users with administrative (wheel) access:${NC}"
    if getent group wheel >/dev/null 2>&1; then
        ...
        print_info "wheel group not found"
    fi
    
    # Check recent sudo commands via journald (Fedora log source)
    if command -v journalctl &>/dev/null; then
        recent_sudo=$(journalctl _COMM=sudo --since "24 hours ago" 2>/dev/null | tail -5 | wc -l)
        if [ "$recent_sudo" -gt 0 ]; then
            print_info "Recent sudo commands detected (last 24h):"
            journalctl _COMM=sudo --since "24 hours ago" 2>/dev/null | tail -5 | while read -r line; do
                echo "  $line"
                echo "  $line" >> "$AUDIT_LOG"
            done
        fi
...
audit_permissions() {
    ...
    declare -A critical_files=(
        ["/etc/passwd"]="644"
        ["/etc/shadow"]="000" # Fedora often restricts shadow completely to root (000 or 640 depending on policy)
        ["/etc/group"]="644"
        ["/etc/gshadow"]="000"
        ["/root"]="700"
        ["/home"]="755"
    ...
    for file in "${!critical_files[@]}"; do
        if [ -e "$file" ]; then
            ...
            if [ "$perms" = "$expected" ] || ([ "$file" = "/etc/shadow" ] && [ "$perms" = "640" ]); then
...
# ──────────────────────────────────────────────
# Check Failed Login Attempts (Using Journald on Fedora)
# ──────────────────────────────────────────────
audit_failed_logins() {
    print_section "Failed Login Attempts"
    
    if command -v journalctl &>/dev/null; then
        failed_count=$(journalctl _COMM=sshd | grep "Failed password" | wc -l)
        echo -e "${BLUE}Failed SSH login attempts (total): $failed_count${NC}"
        
        if [ "$failed_count" -gt 0 ]; then
            print_warning "Multiple failed login attempts detected"
            echo -e "\n${BLUE}Recent failed attempts (last 10):${NC}"
            journalctl _COMM=sshd | grep "Failed password" | tail -10 | while read -r line; do
                echo "  $line"
                echo "  $line" >> "$AUDIT_LOG"
            done
        else
...
audit_open_ports() {
    ...
    if command -v ss &>/dev/null; then
        ...
        print_warning "ss tool not found"
    fi
}
