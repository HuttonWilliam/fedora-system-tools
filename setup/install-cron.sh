#!/bin/bash
# ========================================================================
# Fedora System Tools - Cron Job Installer
...
CRON_BACKUP_DIR="$HOME/.fedora-cron-backups"
CRON_TAG="# fedora-system-tools"
...
${CYAN}Fedora System Tools - Cron Job Installer${NC}
...
OPTIONS:
    ...
    --list     List all currently installed fedora cron jobs
    --remove   Remove all fedora-system-tools cron jobs
...
# ──────────────────────────────────────────────
# List fedora cron jobs
# ──────────────────────────────────────────────
list_cron_jobs() {
    echo -e "\n${CYAN}Current fedora-system-tools cron jobs:${NC}\n"
    local jobs
    jobs=$(crontab -l 2>/dev/null | grep "$CRON_TAG")
    if [ -z "$jobs" ]; then
        print_info "No fedora-system-tools cron jobs found."
    else
        echo "$jobs"
    fi
    echo
...
# ──────────────────────────────────────────────
# Remove all fedora cron jobs
# ──────────────────────────────────────────────
remove_cron_jobs() {
    echo -e "\n${YELLOW}Removing all fedora-system-tools cron jobs...${NC}"
    backup_crontab
    ...
    if echo "$current" | grep -q "$CRON_TAG"; then
        ...
        print_status "All fedora-system-tools cron jobs removed."
    else
        print_info "No fedora-system-tools cron jobs found to remove."
    fi
    echo
}
...
interactive_setup() {
    echo -e "\n${GREEN}╔══════════════════════════════════════════════╗${NC}"
    echo -e "${GREEN}║   Fedora System Tools - Cron Job Installer   ║${NC}"
    echo -e "${GREEN}╚══════════════════════════════════════════════╝${NC}\n"
    ...
    print_info "List fedora jobs: $0 --list"
    print_info "Remove all fedora jobs: $0 --remove"
    echo
}
